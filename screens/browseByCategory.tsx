...
const BrowseByCategory = ({ route }: ScreenProps) => {
  ...
  React.useEffect(() => {
    ...
    let result : Object[] = [];
    result = await getProductsByCategory(route.params.category);
    setItems(result);
  }
console.log(route.params.category)
  ...
  return (
    ...
    <Headline style={styles.headline}>Browse {route.params.category} Items</Headline>

      <ScrollView>
         <View style={styles.cardView}> 
        {items.map((item, i) => { return (
          <Card key={i} style={styles.cardStyle} onPress={()=>{return navigation.navigate("Product", {product: getItem(item)})}}>
            <Card.Cover  source={{ uri: getImage(item) }} />
